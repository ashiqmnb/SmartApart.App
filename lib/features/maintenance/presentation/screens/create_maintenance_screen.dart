...
/// Form to raise a new maintenance request — category dropdown,
/// priority selector, title/description, and a multi-image picker
/// (max 5) with thumbnail previews + delete buttons before submit.
...
class _CreateMaintenanceScreenState extends State<CreateMaintenanceScreen> {
  ...
  Future<void> _pickImages() async {
    ...
    // pickMultiImage lets the user select several at once — same
    // picker instance used elsewhere in the app (image_picker package).
  ...
  Future<void> _handleSubmit() async {
    context.read<MaintenanceProvider>().clearError();
    ...
    if (success && mounted) {
      ...
      // Back to the list — it will show stale data until refreshed,
      // so pop and let the list's own RefreshIndicator/reload handle it.
      context.pop();
